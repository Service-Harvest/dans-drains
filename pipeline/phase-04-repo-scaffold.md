...
12. **CNAME file** for the client's custom domain (used by GitHub Pages),
    placed at `/site/CNAME`. This file is what GitHub's serving layer reads
    from the deployed `/site` artifact to actually serve the custom domain —
    the Phase 13 Pages API binding alone is not sufficient. It must survive
    into every deploy: don't gitignore it or strip it from `/site`. (If a
    build is ever deployed deliberately without it — e.g. a neutralized test
    deploy — the custom domain will not be served even if it's later bound
    via the Pages API; that's the expected consequence, and Phase 13's
    activation step verifies serving against exactly this.)
