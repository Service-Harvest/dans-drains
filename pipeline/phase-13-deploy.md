...
8. **Custom domain connection with canonicalization — a required step, not
   ...
   c. Once the client confirms DNS is added (allow real propagation time —
      ...
      **The API response and `https_enforced` confirm the control-plane
      binding only — they do not prove the site is actually being served on
      the domain.** For this workflow-based deploy, GitHub's serving layer
      honors the `CNAME` file baked into the deployed `/site` artifact (the
      Phase 4 Task 12 file), not the API binding on its own: if that file
      isn't present in the most recent deploy, the `-f cname=` call can
      return success and even report `https_enforced: true` while the domain
      serves the wrong build or nothing — and the next workflow run drops the
      domain outright, because the deployed artifact never claimed it. So
      before treating the domain as live, verify the serving layer, not just
      the API: (i) confirm `/site/CNAME` contains the canonical domain and
      was part of the last successful `deploy-pages` run (it's committed, not
      gitignored or stripped by a neutralized/test deploy); and (ii) fetch
      the canonical domain directly — `curl -s https://[canonical-domain]` —
      and confirm it returns `200` serving *this* client's build, checked
      against a content marker unique to this site (the homepage `<title>` or
      H1), not merely that some page loads (a stale build, or a different
      repo still claiming the domain mid-cutover, can answer on the same URL
      with an `age: 0`/`x-cache: MISS` origin response — a genuine loaded
      page that is nonetheless the wrong site). Treat the domain as activated
      only once the live canonical URL serves this client's content over
      HTTPS. And if the domain is currently an active custom domain on a
      *different* Pages repo (e.g. an earlier build of this client), it can
      be served by only one Pages site at a time — release it there first by
      clearing that repo's Pages `cname` **and** removing the `CNAME` from
      its deployed artifact (redeploy so its next build stops claiming the
      domain), or this binding will report success while the old repo keeps
      serving the domain.
