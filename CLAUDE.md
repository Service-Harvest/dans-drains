...
- **Classifying a mid-build fix — client-specific or template-level — is
  ...
  - **State the classification in every fix-completion report — as a required
    line, without being asked.** Framing this as a standing principle you
    "own" was not forceful enough to change behavior: the domain-binding
    cutover this session was completed and fully reported with no mention of
    classification or syncing until it was explicitly requested — even though
    it was a template-level-plus-client-specific mix, as the after-the-fact
    answer confirmed. So make it procedural, not aspirational: at the end of
    **every** fix, *before* you tell the user it's done, include a short
    **Classification** line in that same report stating one of — **template-
    level** / **client-specific** / **both** (split out which part is which,
    and confirm the template half was ported to `pipeline-template/` and
    verified byte-identical) / **genuinely ambiguous** (name the uncertain
    part per the clause above and ask). A completion report without this line
    is itself incomplete — treat the line as part of the required format, the
    same way "done" is. This applies to *every* kind of fix, not just
    page-level content or CSS bugs: **operational and infrastructure work —
    deploys, domain/DNS/Pages changes, workflow or script edits, anything
    touching a reusable file or mechanism — is exactly as subject to this
    step**, because that category is precisely where the framework has already
    failed to pattern-match. If you are about to write "done," you owe the
    Classification line first.
